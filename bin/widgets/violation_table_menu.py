...
class HiddenRulseRetriveMenu:
    ...
    def __init__(self, 
        ...
        """
        Initialize the HiddenRulseRetriveMenu.
...
class SimpleContextMenu:
    """
    A class to create and manage a simple context menu for a given widget.
    The menu is defined by a data structure and appears at the 
    bottom-left of the parent widget on a right-click.
    """

    def __init__(self, 
                 parent_widget: tk.Widget, 
                 menu_data: List[Union[str, Dict[str, Any]]]
                 ) -> None:
        """
        Initializes the context menu.
        
        :param parent_widget: The widget to bind the right-click event to.
        :param menu_data: A list defining the menu.
                          Example: [{"label": "Copy", "command": copy_func}, "separator"]
        """
        self.parent_widget: tk.Widget = parent_widget
        self.menu_data: List[Union[str, Dict[str, Any]]] = menu_data
        
        # Create the tk.Menu object
        self.menu = tk.Menu(self.parent_widget, tearoff=0)
        self._build_menu()
        
        # Bind the right-click event
        self.parent_widget.bind("<Button-3>", self._show_menu)

    def _build_menu(self) -> None:
        """
        Private method to populate the tk.Menu with items
        from the self.menu_data list.
        """
        for item in self.menu_data:
            if isinstance(item, str) and item == "separator":
                self.menu.add_separator()
            elif isinstance(item, dict):
                label: str = item.get("label", "Missing Label")
                command: Optional[Callable] = item.get("command", None)
                self.menu.add_command(label=label, command=command)

    def _show_menu(self, event: tk.Event) -> None:
        """
        Private event handler to display the context menu
        aligned to the bottom-left of the parent widget.
        """
        widget = event.widget
...
if __name__ == "__main__":
    ...
    root.title("HiddenRulseRetriveMenu Class Example")
    ...
    # --- 1. Define the functions for the menu ---
    def cut_action() -> None:
        print("Cut clicked")
    ...
    # --- 2. Define the menu structure as data ---
    menu_definition = [
        {"label": "Cut", "command": cut_action},
        {"label": "Copy", "command": copy_action},
        {"label": "Paste", "command": paste_action},
        "separator",
        {"label": "Delete", "command": delete_action}
    ]

    # --- 3. Create the widget ---
    action_button = ttk.Button(root, text="Right-Click Me!")
    action_button.pack(pady=50, padx=50) 

    # --- 4. Instantiate the class to attach the menu ---
    # The class handles all the binding and logic
    context_menu = SimpleContextMenu(action_button, menu_definition)
